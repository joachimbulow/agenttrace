...
5. The orchestrator processes every record in the CSV sequentially (each
   running the full gate -> ... -> branch chain). Each record's spans nest
   under its own `primo_record[<record_id>]` span under one pipeline root.
