...
A run is a **container**, not the thing you watch. See [Row](#row).
...
A row is a **projection over spans, not a stored entity**. Concretely, a row is the span
created for a record's own `_GRAPH.ainvoke` call, identified by carrying a `record_id`
attribute. Its `row_id` *is* that span's id. Everything beneath it in the trace tree is that
row's work.

Why this matters: a run of the 7-record sample extract produces ~78 spans, all concurrent. A
row produces ~11, in a legible sequence. The canvas renders a row, never a run. See
[ADR-0002](decisions/0002-row-as-unit-of-observation.md).

The vocabulary is deliberately CSV-flavoured — AgentTrace owns the framing rather than
inventing a generic synonym.
...
status is recorded, which is why the row endpoint returns them inline.

## Attribute

Key-value metadata on a span. `record_id` and `policy_id` are the load-bearing ones: their
presence on a span is what makes it a [Row](#row) root.

## Card

A row's span as rendered on the graph canvas. One card per node. A card has exactly one
...
The only thing the streaming endpoint sends. A ping means *"this row's data changed, refetch
it"* — nothing more. It carries no span data:

```
data: {"row_id": "...", "run_id": "...", "rev": 42}
```
