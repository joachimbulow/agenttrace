"""Integration tests for the row projection and the live invalidation stream.
...
from agent_trace.presentation.routers import rows as rows_router
...
async def _seed_two_rows(client: AsyncClient, run_id: str = "run-1") -> str:
    """A pipeline root with two record rows, one finished and one running."""
    await _ingest(
        client,
        run_id,
        [
            ...
            _span_start("a-gate", "gate", parent_id="row-a", at=2),
            _span_end("a-gate", at=3),
            _span_end("row-a", at=4),
            _span_start(
                "row-b",
            ...
            _span_start("b-gate", "gate", parent_id="row-b", at=2.5),
        ],
    )
    return run_id
...
class TestRowList:
    """GET /api/v1/rows?run_id=..."""

    @pytest.mark.asyncio
    async def test_unknown_run_is_404(self, live_client: AsyncClient) -> None:
        response = await live_client.get("/api/v1/rows", params={"run_id": "nope"})

        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_run_id_is_required(self, live_client: AsyncClient) -> None:
        response = await live_client.get("/api/v1/rows")

        assert response.status_code == 422

    @pytest.mark.asyncio
    async def test_lists_record_rows_not_the_run_root(
        self, live_client: AsyncClient
    ) -> None:
        """The pipeline root is a container, not a row."""
        run_id = await _seed_two_rows(live_client)

        data = (await live_client.get("/api/v1/rows", params={"run_id": run_id})).json()

        assert [r["row_id"] for r in data["rows"]] == ["row-a", "row-b"]
        assert [r["record_id"] for r in data["rows"]] == ["REC-1", "REC-2"]
        assert data["rows"][0]["policy_id"] == "POL-1001"

    @pytest.mark.asyncio
    async def test_status_and_counts_are_derived(self, live_client: AsyncClient) -> None:
        run_id = await _seed_two_rows(live_client)

        rows = (await live_client.get("/api/v1/rows", params={"run_id": run_id})).json()["rows"]
        by_id = {r["row_id"]: r for r in rows}

        assert by_id["row-a"]["status"] == "completed"
        assert by_id["row-a"]["duration_ms"] == 3000.0
        assert by_id["row-a"]["node_count"] == 2
        assert by_id["row-b"]["status"] == "running"
        assert by_id["row-b"]["ended_at"] is None

    @pytest.mark.asyncio
    async def test_error_event_beats_completion(self, live_client: AsyncClient) -> None:
        """A row whose root closed but which contains a failure is an error."""
        run_id = await _seed_two_rows(live_client)
        await _ingest(live_client, run_id, [_span_event("a-gate", "error", at=3)])

        rows = (await live_client.get("/api/v1/rows", params={"run_id": run_id})).json()["rows"]

        assert next(r for r in rows if r["row_id"] == "row-a")["status"] == "error"

    @pytest.mark.asyncio
    async def test_falls_back_to_root_children_without_record_ids(
        self, live_client: AsyncClient
    ) -> None:
        """A program traced with the bare SDK still gets rows, not an empty list."""
        await _ingest(
            live_client,
            "run-plain",
            [
        ...
        rows = (
            await live_client.get("/api/v1/rows", params={"run_id": "run-plain"})
        ).json()["rows"]

        assert [r["row_id"] for r in rows] == ["s1", "s2"]
        assert rows[0]["record_id"] is None

    @pytest.mark.asyncio
    async def test_single_span_run_is_its_own_row(self, live_client: AsyncClient) -> None:
        await _ingest(
            live_client,
            "run-solo",
        ...
        rows = (
            await live_client.get("/api/v1/rows", params={"run_id": "run-solo"})
        ).json()["rows"]

        assert [r["row_id"] for r in rows] == ["only"]
...
class TestRowTree:
    ...
    @pytest.mark.asyncio
    async def test_unknown_row_is_404(self, live_client: AsyncClient) -> None:
        response = await live_client.get("/api/v1/rows/nope")

        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_returns_only_that_rows_subtree(self, live_client: AsyncClient) -> None:
        """Sibling rows must not leak into the canvas."""
        await _seed_two_rows(live_client)

        data = (await live_client.get("/api/v1/rows/row-a")).json()

        assert data["row_id"] == "row-a"
        assert data["run_id"] == "run-1"
        assert data["root"]["id"] == "row-a"
    ...
    async def test_children_ordered_by_start_time(self, live_client: AsyncClient) -> None:
        ...
        await _ingest(
            ...
            "run-order",
            [
                _span_start("row", "row", span_type="agent_run", at=0, attributes={"record_id": "R"}),
                _span_start("late", "late", parent_id="row", at=9),
                _span_start("early", "early", parent_id="row", at=1),
                _span_start("mid", "mid", parent_id="row", at=5),
            ],
        )

        data = (await live_client.get("/api/v1/rows/row")).json()
...
class TestRowStreamRoute:
    """GET /api/v1/rows/{row_id}/events -- the route around the generator.
    ...
    @pytest.mark.asyncio
    async def test_unknown_row_is_404(self, live_client: AsyncClient) -> None:
        response = await live_client.get("/api/v1/rows/nope/events")
    ...
    async def test_sets_streaming_headers(self, live_client: AsyncClient) -> None:
        ...
        response = await rows_router.stream_row_events(
            row_id="row-a",
    ...
    @pytest.mark.asyncio
    async def test_resolves_the_row_without_holding_a_session(
        self, live_client: AsyncClient
    ) -> None:
        """SQLite has one pooled connection; a held session would deadlock ingest."""
        await _seed_two_rows(live_client)

        await rows_router.stream_row_events(
            row_id="row-a",
            request=_StubRequest(),  # type: ignore[arg-type]
            bus=get_run_event_bus(),
        )
...
class TestPingStream:
    ...
    async def test_emits_initial_ping_immediately(self) -> None:
        ...
        stream = rows_router._ping_stream(_StubRequest(), bus, "row-a", "run-1")  # type: ignore[arg-type]
    ...
    async def test_heartbeat_is_a_data_frame_not_a_comment(
        ...
        """
        monkeypatch.setattr(rows_router, "KEEPALIVE_SECONDS", 0.01)
        bus = InMemoryRunEventBus()
        stream = rows_router._ping_stream(_StubRequest(), bus, "row-a", "run-1")  # type: ignore[arg-type]
        await anext(stream)
    ...
    async def test_stops_when_the_client_disconnects(
        ...
        bus = InMemoryRunEventBus()
        stream = rows_router._ping_stream(request, bus, "row-a", "run-1")  # type: ignore[arg-type]
        await anext(stream)
    ...
    @pytest.mark.asyncio
    async def test_sibling_row_activity_also_pings(self) -> None:
        ...
        pending = asyncio.create_task(anext(stream))  # type: ignore[arg-type]
        await asyncio.sleep(0)
        # A change to row-b, in the same run.
        await bus.publish("run-1")

        assert _parse(await asyncio.wait_for(pending, timeout=2))["row_id"] == "row-a"
        await stream.aclose()
...
class TestPublishOrdering:
    ...
    async def test_data_is_present_by_the_time_a_ping_arrives(
        ...
        await _ingest(
            live_client,
            "run-1",
            [_span_start("a-judge", "judge", parent_id="row-a", at=5)],
        )
        await asyncio.wait_for(pending, timeout=5)

        fetched = (await live_client.get("/api/v1/rows/row-a")).json()
        await stream.aclose()
